...
```mermaid
graph TB
    INPUT["💡 你的 APP 构想<br/><i>用日常语言描述即可</i>"]

    ORCH["🧠 Orchestrator 总调度器<br/><i>读取状态 → 判断下一步 → 路由 → 追踪</i>"]

    INPUT --> ORCH

    subgraph PHASE0["📋 PHASE 0 — 规划阶段：6 个智能体，严格串行"]
        P0_1["① 📝 PRD 专家<br/>产品需求文档"] -->
        P0_2["② 🏗️ 技术架构师<br/>技术选型与目录结构"] -->
        P0_3["③ 📏 编码规范专家<br/>代码质量规则"] -->
        P0_4["④ 🗄️ Schema 架构师<br/>数据库设计"] -->
        P0_5["⑤ 🔌 API 契约架构师<br/>接口规范"] -->
        P0_6["⑥ 📋 任务拆分专家<br/>构建计划"]
    end

    ORCH --> P0_1

    REVIEW0["🔍 跨文档审查<br/><i>新对话，零上下文偏见</i>"]
    ...
    REVIEW1["🔍 里程碑审查<br/><i>新对话，零'必须修复'项</i>"]

    P1_VERIFY -- "通过" --> REVIEW1

    DONE["🚀 完整可运行的 APP<br/><i>Expo + Supabase · React Native</i>"]

...
```mermaid
graph TB
    INPUT["💡 用户输入<br/><i>用日常语言描述你的 APP 构想</i>"]

    INPUT --> ORCH

    MEMORY["🗄️ Memory<br/>外部记忆文件<br/><i>状态 · 进度 · 决策 · 长期记忆</i>"]
    ORCH["🧠 Orchestrator 总调度器<br/><i>读取状态 → 判断 → 路由 → 追踪</i>"]
    MEMORY -. "提取" .-> ORCH
    ORCH -. "更新" .-> MEMORY

    ORCH --> P0_1

    subgraph PHASE0["📋 PHASE 0 — 规划阶段：6 个 Agent，严格串行"]
        direction LR
        P0_1["① 📝<br/>产品方案设计专家<br/><i>→ 产品需求文档</i>"]
        P0_2["② 🏗️<br/>技术架构师<br/><i>→ 技术方案</i>"]
        P0_3["③ 📏<br/>编码规范专家<br/><i>→ 代码规范文档</i>"]
        P0_4["④ 🗄️<br/>数据库设计专家<br/><i>→ Schema 文档</i>"]
        P0_5["⑤ 🔌<br/>API 接口设计专家<br/><i>→ 接口文档</i>"]
        P0_6["⑥ 📋<br/>编码任务拆分专家<br/><i>→ 分阶段编码任务书</i>"]
        P0_1 --> P0_2 --> P0_3 --> P0_4 --> P0_5 --> P0_6
    end

    P0_6 --> REVIEW0["🔍 规划审查<br/><i>新对话 · 零上下文偏见</i>"]

    REVIEW0 --> P1_DB

    subgraph PHASE1["💻 PHASE 1-N — 构建阶段：6 个 Agent，混合串并行"]
        P1_DB["🗃️ Agent-DB<br/>建表 + RLS 权限策略"]
        P1_FE["🎨 Agent-FE<br/>前端页面 + Mock"]
        P1_BE["🔧 Agent-BE<br/>后端 Edge Function"]
        P1_CONNECT["🔗 Agent-CONNECT<br/>Mock → 真实 API 联调"]
        P1_VERIFY["✅ Agent-VERIFY<br/>验收测试"]
        P1_FIX["🩹 Agent-FIX<br/>精确修复补丁"]
    ...
    P1_VERIFY -- "✅ 通过" --> REVIEW1["🔍 里程碑审查<br/><i>新对话 · 零'必须修复'项</i>"]

    REVIEW1 --> DONE["🚀 完整可运行的 APP<br/><i>React Native · Expo + Supabase · 全栈交付</i>"]
...
| 智能体 | 模型 | 职责 |
|-------|------|------|
| 🧠 **Orchestrator** | Claude Opus 4.7 | 读取项目状态 → 判断下一步 → Tool Use 动态路由 → 更新进度 |
